...
const StudentMessages = ({ studentDetails }) => {
    ...
    const selectTeacherFormHandler = async (e) => {
        ...
        e.preventDefault()
        try {
            const { data } = await axios.get(`/api/messages/${e.target.teacher.value}`)
            console.log(data)
            setSelectedTeacher(e.target.teacher.value)
    ...
    const sendMessageHandler = async(e) => {
        e.preventDefault()
        try {
            const { data } = await axios.post(`/api/messages/send/${selectedTeacher}`, {
            ...
            <div className="flex items-center justify-end space-x-4 w-full border-b border-gray-200 pb-5">
                {onClickSendNewMsg ? (
                    ...
                    <div className="flex items-center space-x-4">
                        ...
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-6 h-6 text-white group-hover:text-blue-700">
